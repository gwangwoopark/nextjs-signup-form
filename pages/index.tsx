...
const schema = yup.object().shape({
  ...
  password: yup
    ...
    .matches(
      /^.{8,16}$/,
      "비밀번호는 8~16자이며, 영문 소문자, 영문 대문자, 숫자, 특수문자를 모두 포함해야 합니다."
    )
    .matches(
      /^(?=.*[a-z]).{8,16}$/,
    ...
    .matches(
      /^(?=.*[A-Z]).{8,16}$/,
      "비밀번호는 영문 대문자를 포함해야 합니다."
    )
    .matches(/^(?=.*\d).{8,16}$/, "비밀번호는 숫자를 포함해야 합니다.")
    .matches(
      /^(?=.*[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"]).{8,16}$/,
              ...
              <div className={"mb-4"}>
                {errors.email && (
                  <div className="absolute">
                    <span className="relative bottom-3 left-3 px-2 text-xs tracking-wide text-red-600 bg-white">
                ...
                <div>
                  <input
                    ...
                    type="email"
                    className={cls(
                      "form-control block w-full px-4 text-lg font-normal text-gray-700 bg-white bg-clip-padding border border-solid rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none",
